...
describe('GET /todos/:id', () => {
    ...
    it('should not return todo of other user', (done) => {
        request(app).get(`/todos/${todos[0]._id.toHexString()}`)
...
describe('PATCH /todos/:id', () => {
    it('should update the todo', (done) => {
        ...
        let completed = true;
        request(app).patch(`/todos/${id.toHexString()}`).set('x-auth', users[0].tokens[0].token)
            .send({
                text,
                completed
            })
    ...
    it('should clear completedAt when todo is not completed', (done) => {
        let id = todos[1]._id;
        let completed = !true;
        request(app).patch(`/todos/${id.toHexString()}`).set('x-auth', users[1].tokens[0].token).send({
                completed
            }).expect(200)
            .expect((res) => {
